...
class LMProposalModelSelector(BaseModelSelector):
    ...
    def _build_prompt(self, preview: List[Dict[str, Any]]) -> str:
        ...
        return (
            ...
            '{"combinations": [[idx_for_node0, idx_for_node1, ...], ...]}\n'
            "Rules:\n"
    ...
    def _parse_proposed_indices(
        self, text: str, all_index_combos: Sequence[Tuple[int, ...]]
    ) -> List[Tuple[int, ...]]:
        if not text.strip():
            return []
        try:
        ...
        try:
            response = self.proposer_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a precise model-selection proposer. Return JSON only.",
                    },
                    {"role": "user", "content": prompt},
                ],
            )
            raw = self._content_to_text(response.choices[0].message.content)
            proposed = self._parse_proposed_indices(raw, all_index_combos)
