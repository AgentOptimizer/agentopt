...
**`select_best()` parameters (all selectors):**

| Parameter | Type | Description |
|-----------|------|-------------|
| `parallel` | `bool` | **Model-level parallelism** — evaluate different model combinations concurrently (default: `False`) |
| `max_concurrent` | `int` | **Question-level parallelism** — max concurrent API calls per combination when evaluating the dataset asynchronously (default: implementation-dependent, usually `20`) |

### Selection Strategies

AgentOpt includes several model selection strategies:

- **`BruteForceModelSelector`** (default, aliased as `ModelSelector`) — Grid search over the full Cartesian product of all candidate model combinations. Thorough but scales as O(n^k) where n is models per proxy and k is the number of proxies. When `parallel=True`, the effective batch size is all combinations that can be scheduled up to the available workers.

- **`RandomSearchModelSelector`** — Random subset search over the Cartesian product: samples a fraction of all combinations and evaluates only that subset. Useful when brute force is too expensive but you still want broad coverage. When `parallel=True`, the batch consists of all sampled combinations that can be scheduled up to the available workers.

-- **`HillClimbingModelSelector`** — Local-search / hill-climbing strategy using model quality/speed rankings. Starts from an initial combination and iteratively swaps one model at a time, keeping improvements. Uses **batched neighbors** per iteration, with batch size explicitly controlled by a `batch_size` parameter (independent of worker count). Small values like 2–8 are recommended to preserve adaptivity.

- **`ArmEliminationModelSelector`** — Bandit-style successive elimination strategy that evaluates combinations in rounds with growing batch sizes and drops statistically dominated arms using confidence bounds. In each round/stage, the batch is exactly the set of **all remaining (non-eliminated) combinations** in that stage.

- **`HyperbandModelSelector`** — Bandit-style full Hyperband algorithm over the dataset, treating the number of samples as resource and running multiple successive-halving brackets with different starting budgets. For each bracket and stage, the batch is the set of **all active configurations in that stage** determined by the Hyperband schedule.

- **`BayesianOptimizationModelSelector`** — Gaussian process-based optimization that models the accuracy surface over model combinations and uses an acquisition function to select the most promising combination to evaluate next. When `parallel=True`, it proposes a **user-specified batch** of candidates per BO iteration via a `batch_size` parameter; small values such as 2, 4, or 8 are typical.
...
AgentOpt separates two kinds of parallelism:

- **Question-level parallelism (always on):**  
  Every selector evaluates each chosen model combination against the dataset using asynchronous calls with a semaphore limit (`max_concurrent`). This means multiple dataset items are evaluated concurrently for a given combination by default.

- **Model-level parallelism (`parallel=True`):**  
  When `parallel=True`, supported selectors also evaluate **different model combinations concurrently** (on top of question-level parallelism).

All current selectors (`BruteForceModelSelector`, `RandomSearchModelSelector`, `HillClimbingModelSelector`, `ArmEliminationModelSelector`, `HyperbandModelSelector`, and `BayesianOptimizationModelSelector`) use question-level parallelism by default. Model-level parallelism is enabled via `parallel=True` where applicable (all except hill climbing, which remains sequential at the model level by design).
...
MIT
>>>>>>> 675ac7e (update README about parallelism descriptions)
